...
export function generateToken(data: Record<string, any>, tokenType: TokenType) {
    const token = jwt.sign(
        ...
        {
            expiresIn: tokenType == 'access' ? `${apiConfig.access_expire}m` as string : `${apiConfig.refresh_expire}d`,
...
export async function verifyToken(token: string, tokenType: TokenType) {
    try {
        const tokenData = jwt.verify(token, tokenType === 'access' ? apiConfig.access_token : apiConfig.refresh_token);
        return tokenData;
    } catch (e) {
        return false;
    }
